...
void BFS(vector<int> adj[], int v, int s)
{
  ...
  q.push(s);
   //enqueue the source vertex into the queue (in tree we enqueue root, here source as graph can be directional)

//in tree we used to run loop until queue gets empty,
//but here graph can be directional, so we run until it gets false (not NULL as in tree)
  while(q.empty()==false){
    int u=q.front();
    q.pop();
    cout<<u<<" ";
