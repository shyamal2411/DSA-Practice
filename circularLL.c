...
void recusivedisplay(struct node * p)
{
 ...
 head node that is the first node, it has to know that this is the second cycle & now program has to stop,
 so flag will increament & program won't move further*/
	{
...
int delete(struct node *p, int index)
{
	...
	if (index < 0 || index > length(head)) //to cover invalid response
		return -1;

	if (index == 1)//means,if position is 1
		/*this is not an array,so index 1 thi chaalu kari sakay,
		all depends on us,whether to start from 0 or 1*/
	{
		while (p->next != head)
			p = p->next;
		x = head->data; //data is assigned to x just to store the data & delete the node
		//uptil this is for the first node
		/*NOTE:::: DONOT THINK TO WRITE free(head); AT THIS POINT,OUTPUT WILL GIVE GARBAGE VALUE,DON'T KNOW THE REASON*/
		if (head == p) //if there is the only node in the list
		{
			free(head);
			head = NULL;
		}
		...
		{
			p->next = head->next;//in order to delete the node,p's next is shifted to next
			free(head);
			head = p->next;
		}
	}
...
int main()
{
	...
	create(a, 5);
	delete(head, 1);
	/*change the value in delete(head, value);
	and observe the change happening in output*/
	display(head);
	return 0;
}
