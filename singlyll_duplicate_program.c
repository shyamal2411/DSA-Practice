...
void insert(struct node *p, int index, int x)
{
	...
	if (index == 0)
	{
		t->next = first;
		first = t;
		first = last;//added it with self doubt,remove if not working properly
	}
	else
	{
...
void duplicate(struct node *p)
{
	...
	while (q != NULL)
	{
		if (p->data != q->data)
		{
			p = q;//p ne q ni jagya dai devani ne pchi  q ne p na next aa transverse
			q = q->next;
		}
		else
		{
...
int main()
{
	...
	int a[] = {21, 10, 20, 30, 40, 50};
	//int b[] = {5, 15, 20, 35, 45}; for concat & merge
	create(a, 6);
	insert(first, 3, 70);
	display(first);
	/*t1 = first->next->next;
